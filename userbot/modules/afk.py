        ...
        try:
            if pic.endswith((".tgs", ".webp")):
                ...
                shites = await bot.send_message(
                    event.chat_id,
                    f"🔥 **ONLINE**\n**Kyud {ALIVE_NAME} Kembali Online Untuk Chat Telegram**\n**Sejak :** `{total_afk_time}` **Yang Lalu**",
                )
            else:
                shite = await bot.send_message(
                    event.chat_id,
                    f"**Kyud {ALIVE_NAME} Sedang AFK!**\n\n**Sejak :** `{total_afk_time}` **Yang Lalu**",
                    file=pic,
                )
        except BaseException:
            shite = await bot.send_message(
                event.chat_id,
                f"🔥 **ONLINE**\n**Kyud {ALIVE_NAME} Kembali Online**\n**Sejak :** `{total_afk_time}` **Yang Lalu**",
            )
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:
        ...
        if reason:
            ...
            message_to_reply = (
                f"**Maaf Kyud {ALIVE_NAME} Sedang AFK**\n\n**Sejak :** `{total_afk_time}` **Yang Lalu**"
            )
        try:
            if pic.endswith((".tgs", ".webp")):
