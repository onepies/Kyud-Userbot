...
async def _(event):
    ...
    if not event.reply_to_msg_id:
        await event.edit("```Kyud, Mohon Balas Ke Pesan Pengguna atau ketik .deteksi (ID/Username) Yang mau Anda deteksi```")
        return
    if input_str:
        try:
            uid = int(input_str)
            ...
            try:
                ...
                await edit.event("`Kyud, Mohon Berikan ID/Username untuk menemukan Riwayat`"
                                 )
            uid = u.id
    else:
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            await steal.reply(
                "``Kyud Mohon Unblock @tgscanrobot Dan Coba Lagi```"
            )
