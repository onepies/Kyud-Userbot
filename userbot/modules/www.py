...
async def pingme(pong):
    ...
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    await pong.edit("`.....⚡Kyud⚡.....`")
    await pong.edit("`⚡️`")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await pong.edit(f"•⎚• ⎆ __kyud__ **Pong!**\n"
                    f"☞  __Ping:__ "
                    f"`%sms` \n"
                    f"☞  __Sisa Waktu:__ "
                    f"`{uptime}` \n" % (duration))
...
async def pingme(pong):
    ...
    duration = (end - start).microseconds / 1000
    await pong.edit(f"卍════〠 **KYUD USERBOT** 〠════卍\n"
                    f"═⎆ **Ping:** "
                    f"`%sms` \n"
                    f"═⎆ **Sisa Waktu:** "
                    f"`{uptime}` \n"
                    f"**✠➲ ᴋʏᴜᴅ:** `{ALIVE_NAME}`" % (duration))
...
async def pingme(pong):
    ...
    duration = (end - start).microseconds / 1000
    await pong.edit(f"- K Y U D -\n"
                    f"**☞ sinyal  :** "
                    f"`%sms` \n"
                    f"**☞ i'm online  :** "
                    f"`{uptime}` \n"
                    f"__|━|⎆__ **My Kyud  :** `{ALIVE_NAME}`" % (duration))
...
async def pingme(pong):
    ...
    duration = (end - start).microseconds / 1000
    await pong.edit(f"**Kyud** - {ALIVE_NAME}\n\n"
                    f"**Pong !!** "
                    f"`%sms` \n"
                    f"**Uptime** - "
                    f"`{uptime}` \n" % (duration))
...
async def pingme(pong):
    ...
    await pong.edit("⎚⎆ __Kyud__ **Test Ping!**\n`%sms`" % (duration))
